...
# ── 2. Extract colors with frequency count ────────────────────────
...
def extract_colors_with_count(node, color_counter=None):
    ...
    for fill in node.get("fills", []):
        if fill.get("type") == "SOLID" and "color" in fill:
            c = fill["color"]
            color_counter[rgba_to_hex(c["r"], c["g"], c["b"])] += 1
...
# ── 3. Extract text styles grouped by size+weight+family ──────────
FONT_WEIGHT_MAP = {
    100: "FontWeight.w100",
    200: "FontWeight.w200",
    300: "FontWeight.w300",
...
def extract_text_styles(node, style_map=None):
    """
    Group by (fontSize, fontWeight, fontFamily).
    Count color frequency per group → pick most common color.
    """
    ...
    if node.get("type") == "TEXT":
        ...
        for fill in fills:
            if fill.get("type") == "SOLID" and "color" in fill:
                c = fill["color"]
                color_hex = rgba_to_hex(c["r"], c["g"], c["b"])
                break
...
def write_dart_colors(color_counter):
    ...
    for hex_color in sorted(top_colors):
        count = color_counter[hex_color]
        lines.append(
            f"  static const Color c{hex_color} = Color(0xFF{hex_color}); // used {count}x"
        )
    ...
    for hex, count in color_counter.most_common(5):
        print(f"  c{hex} → used {count}x")

    return set(top_colors)


# ── 5. Write app_fonts.dart ───────────────────────────────────────
def write_dart_fonts(style_map, valid_colors):
    ...
    for (font_size, font_weight, font_family) in sorted(
        ...
        # Pick most used color that exists in our top colors list
        ...
        # Fallback to most common color even if not in top list
        ...
        color_ref = f"AppColors.c{most_common_color}" if most_common_color else "AppColors.c000000"
