...
def single_repo_standup():
    ...
    # skip weekend — go back to last day with commits
    if today.weekday() == 0:        # Monday → check Sunday, Saturday, Friday
        for days_back in [1, 2, 3]:
            candidate = today - timedelta(days=days_back)
            commits = get_commits(path, author,
                f"{format_date(candidate)} 00:00",
                f"{format_date(candidate)} 23:59")
            if commits:
                yesterday = candidate
                break
        else:
            yesterday = today - timedelta(days=3)  # fallback to Friday
    elif today.weekday() == 6:      # Sunday → go back to Saturday or Friday
...
def main():
    ...
    while True:
        ...
        choice = input("\n  Choose: ").strip().replace('\x00', '').replace('\r', '')
